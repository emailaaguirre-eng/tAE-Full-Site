/**
 * WordPress ArtKey Editor - Full UI (ported from Next.js)
 * Palette:
 *   Primary: #FFFFFF
 *   Alt: #ECECE9
 *   Accent: #353535
...
// REST endpoints from localized script
const wpRestApi = window.ArtKeyEditor?.rest || {
  save: '/wp-json/artkey/v1/save',
  get: '/wp-json/artkey/v1/get',
  upload: '/wp-json/artkey/v1/upload',
};
const wpNonce = window.ArtKeyEditor?.nonce || '';
...
export default function ArtKeyEditorWP({ artkeyId = null }) {
  const urlParams = useMemo(() => new URLSearchParams(window.location.search), []);
  const productId = urlParams.get('product_id') || null;
  const fromCustomize = urlParams.get('from_customize') === 'true';

  // Core state
  const [designMode, setDesignMode] = useState(null); // null | 'template' | 'custom'
  ...
  // ArtKey data
  const [artKeyData, setArtKeyData] = useState({
  ...
  const loadArtKey = async (id) => {
    try {
      const res = await fetch(`${wpRestApi.get}/${id}`, { headers: { 'X-WP-Nonce': wpNonce } });
      if (!res.ok) return;
  ...
  const getColorsForPage = (page, arr) => arr.slice(page * 12, page * 12 + 12);
  ...
  const handleImageUpload = async (e) => {
    ...
    for (const file of Array.from(files)) {
      ...
      try {
        const res = await fetch(wpRestApi.upload, { method: 'POST', headers: { 'X-WP-Nonce': wpNonce }, body: formData });
        if (!res.ok) continue;
  ...
  const handleVideoUpload = async (e) => {
    ...
    try {
      const res = await fetch('/wp-json/artkey/v1/upload', { method: 'POST', headers: { 'X-WP-Nonce': wpNonce }, body: formData });
      if (!res.ok) return;
      const result = await res.json();
  ...
  const handleSaveAndCheckout = async () => {
    try {
      const res = await fetch(wpRestApi.save, {
        method: 'POST',
  ...
  // Drag reorder
  const handleFeatureDragStart = (index) => setDraggedFeature(index);
  const handleFeatureDragOver = (e, index) => {
    e.preventDefault();
...
function Card({ title, step, children, onBack }) {
  return (
...
function Carousel({ page, setPage, total, children, labelPrefix }) {
  return (
    <div>
...
function PrimaryButton({ onClick, children, icon, accent }) {
  return (
    <button
      onClick={onClick}
